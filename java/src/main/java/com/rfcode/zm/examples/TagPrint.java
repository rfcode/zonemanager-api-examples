...
public class TagPrint extends Command {
    @Parameter(required = true, description="One or more tag IDs to print")
    ...
    protected void onRun(Http http) throws Exception {
        ...
        for (int i = 0; i < _parameters.size(); i++) {
            query.put("tagid" + i, _parameters.get(i));
        }
        http.get("api/tagprint", query);
    }
}
